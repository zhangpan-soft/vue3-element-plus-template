...
class HttpRequestBuilder implements IHttpUri, IHttpParam, IHttpBody, IHttpRequestBuilder {
    ...
    params(nameValues?: Record<string, any>): IHttpRequestBuilder {
        ...
        for (let nameValuesKey in nameValues) {
...
const defaultRequestInterceptor: IHttpInterceptor = (request: IHttpRequest, response: IHttpResponse) => {
...
class HttpRequest implements IHttpRequest {
    ...
    private request(requestId: string, url: string, method: IHttpMethod, params: Record<string, any>, parts: Record<string, any>, headers: Record<string, string>, cookies: Record<string, string>, requestConfig: IHttpRequestConfig, proxy: IHttpProxy| undefined, resolve: any, reject: any): void{
        try {
            LoggerUtils.debug(`request-${requestId}`, url, method, params, parts, headers, cookies, requestConfig);
            // 处理url参数
            ...
            const request = apiRequest?.request(requestOptions, (response: http.IncomingMessage) => {
                ...
                response.on("end", () => {
                    try { // 拼接数据体
                        ...
                        if (!ObjectUtils.isEmpty(response.headers['content-encoding']) && response.headers['content-encoding'] === "gzip") {
                        ...
                        if (this.httpResponse.status() === 302 && this.options.requestConfig.autoRedirect) {
                            this.request(requestId, response.headers.location as string, method, {}, {}, headers, this.httpResponse.cookies(), requestConfig, proxy, resolve, reject);
                        } else {
    ...
    doRequest(): Promise<IHttpResponse> {
        return new Promise<IHttpResponse>((resolve: any, reject: any) => {
            this.request(uuidv4(), this.options.url as string, this.options.method as IHttpMethod, this.options.params, this.options.parts, this.options.headers, this.options.cookies, this.options.requestConfig,this.options.proxy, resolve, reject);
        })
    }
}
